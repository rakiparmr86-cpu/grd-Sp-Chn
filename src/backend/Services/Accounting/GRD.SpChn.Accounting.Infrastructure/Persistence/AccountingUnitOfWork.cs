...
internal sealed class AccountingUnitOfWork(IDbConnectionFactory connectionFactory)
    : IAccountingUnitOfWork
{
    ...
    internal DbConnection Connection => _connection
        ?? throw new InvalidOperationException("No Accounting transaction is active.");
